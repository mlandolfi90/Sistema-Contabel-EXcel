...
- **Filas 6-19 BLOQUE 1:** lotes con generación de utilidad.
- **Filas 20-24 BLOQUE 2:** participación por socio.
- **Filas 27-34 BLOQUE 3:** asistente de pacto.
- Orden: posición 25.
...
|---|---|---|---|
| 1 | B4 | espeja `SALDOS_Y_ENTIDADES!AA3` | Total pendiente |
| 2 | A8 | Derrame FILTER de `tb_mayor` con `Categoria="Generación Utilidad"` | Lotes con utilidad |
| 3 | B22 | *(R4)* `=SUMPRODUCT((tb_mayor[Cuenta]="Utilidad Distribuible")*(tb_mayor[Categoria]="Generación Utilidad")*(ISNUMBER(SEARCH(A22,tb_mayor[Socios_Participantes]))))` | Cantidad lotes donde participó Socio01 |
...
| 11 | B34 | `=IF(ABS(B32-B33)<0.01,"✓ Cuadra","✗ Descuadre: ...")` | Indicador cuadre |
...
- `Módulo1.GenerarAsientoPacto` lee `B29:B31` y prellena el asiento de pacto.
- Requiere `|Total - Pendiente| ≤ 0.01`.

## Riesgos detectados

1. ~~**Socios hardcoded en B22/B23/B24**~~ → **Corregido en R4**.
2. ~~**Inputs B29:B31 sin validación**~~ → **Corregido en R5**.
